...
class CircleIndicator: LinearLayout {
    ...
    private val temp = TypedValue.applyDimension(
        TypedValue.COMPLEX_UNIT_DIP, 2.0f, resources.displayMetrics)
    ...
    fun createDotPanel(count: Int, defaultCircle: Int, selectCircle: Int, position: Int){
        ...
        val width = mContext?.resources?.displayMetrics?.widthPixels?.minus(200)
        val div = width?.div(13) as Int

        imageDot[0].layoutParams.width = div * 5
        imageDot[0].scaleType = ImageView.ScaleType.FIT_XY
        imageDot[1].layoutParams.width = div * 4
        imageDot[1].scaleType = ImageView.ScaleType.FIT_XY
        imageDot[2].layoutParams.width = div * 3
        imageDot[2].scaleType = ImageView.ScaleType.FIT_XY
